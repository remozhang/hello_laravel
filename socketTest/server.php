...
class WebSocket {
    ...
    private function doServer() {
        $write = $except = NULL;
        $sockets = self::arrayColumn($this->sockets, 'resource');
    ...
    private function dealMsg($socket, $recv_msg) {
        ...
        switch ($msg_type) {
            case 'login':
                ...
                // 取得最新的名字记录
                $user_list = self::arrayColumn($this->sockets, 'uname');
                $response['type'] = 'login';
    ...
    private static function arrayColumn($array, $columnName) {
...
$ws = new WebSocket("192.168.10.10", "4000");
